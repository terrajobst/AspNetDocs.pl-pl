...
---
# <a name="aspnet-signalr-hubs-api-guide---net-client-signalr-1x"></a>Przewodnik interfejsu API centrów sygnałów ASP.NET — klient platformy .NET (sygnalizujący 1. x)
...
- [Przewodnik interfejsu API centrów sygnałów — klient JavaScript](../guide-to-the-api/hubs-api-guide-javascript-client.md)
...
Obiekt serwera proxy jest bezpieczny wątkowo. W rzeczywistości w przypadku wywołania `HubConnection.CreateHubProxy` wiele razy z tą samą `hubName`można uzyskać ten sam buforowany obiekt `IHubProxy`.
...
Aby uzyskać więcej informacji, zobacz [Omówienie i obsługa zdarzeń okresu istnienia połączenia w programie sygnalizującym](../guide-to-the-api/handling-connection-lifetime-events.md).
