...
> 
> 
> - <span data-ttu-id="4d9a7-114">ASP.NET Web Pages (Razor) 3</span><span class="sxs-lookup"><span data-stu-id="4d9a7-114">ASP.NET Web Pages (Razor) 3</span></span>
>   
> 
...
<span data-ttu-id="4d9a7-137">Wewnątrz bloku kodu każda instrukcja kompletny kod musi być zakończona średnikiem.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-137">Inside a code block, each complete code statement must end with a semicolon.</span></span> <span data-ttu-id="4d9a7-138">Wbudowane wyrażenia nie kończy się średnikiem.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-138">Inline expressions don't end with a semicolon.</span></span>
...
<span data-ttu-id="4d9a7-170">Wynik wyświetlany w przeglądarce (po kliknięciu przycisku **przesyłania**):</span><span class="sxs-lookup"><span data-stu-id="4d9a7-170">The result displayed in a browser (after clicking **Submit**):</span></span>
...
1. <span data-ttu-id="4d9a7-184">W edytorze, Utwórz nowy plik i nadaj mu nazwę *AddNumbers.cshtml*.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-184">In your editor, create a new file and name it *AddNumbers.cshtml*.</span></span>
...
<span data-ttu-id="4d9a7-283">W przypadku znaczników prawdopodobnie wiesz, można utworzyć komentarz HTML:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-283">For markup, as you probably know, you can create an HTML comment:</span></span>
...
<span data-ttu-id="4d9a7-367">Oto przykład, aby lepiej zrozumieć różnice:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-367">Here's an example to help you understand the differences:</span></span>
...
<span data-ttu-id="4d9a7-395">Możesz użyć `~` operator zarówno w kodzie serwera (jak powyżej), jak i znaczników w następujący sposób:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-395">You can use the `~` operator both in server code (as above) and in markup, like this:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample48.html)]

<span data-ttu-id="4d9a7-396">W znaczniku, użyj `~` operatora do utworzenia ścieżki do zasobów, takich jak pliki obrazów, innych stron sieci web i plików CSS.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-396">In markup, you use the `~` operator to create paths to resources like image files, other web pages, and CSS files.</span></span> <span data-ttu-id="4d9a7-397">Po uruchomieniu strony ASP.NET przegląda strony (kodu i znaczników) i usuwa wszystkie `~` odwołania do danej ścieżce.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-397">When the page runs, ASP.NET looks through the page (both code and markup) and resolves all the `~` references to the appropriate path.</span></span>

## <a name="conditional-logic-and-loops"></a><span data-ttu-id="4d9a7-398">Logika warunkowe i pętle</span><span class="sxs-lookup"><span data-stu-id="4d9a7-398">Conditional Logic and Loops</span></span>

<span data-ttu-id="4d9a7-399">Kod serwera ASP.NET umożliwia wykonywanie zadań na podstawie warunków i napisać kod, który powtarza instrukcji określoną liczbę razy (czyli kodu wykonywanego w pętli).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-399">ASP.NET server code lets you perform tasks based on conditions and write code that repeats statements a specific number of times (that is, code that runs a loop).</span></span>

### <a name="testing-conditions"></a><span data-ttu-id="4d9a7-400">Warunki badania</span><span class="sxs-lookup"><span data-stu-id="4d9a7-400">Testing Conditions</span></span>

<span data-ttu-id="4d9a7-401">Aby przetestować warunku prostego, możesz użyć `if` instrukcję, która zwraca wartość PRAWDA lub FAŁSZ, oparte na test, należy określić:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-401">To test a simple condition you use the `if` statement, which returns true or false based on a test you specify:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample49.cshtml)]

<span data-ttu-id="4d9a7-402">`if` — Słowo kluczowe uruchamia blok.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-402">The `if` keyword starts a block.</span></span> <span data-ttu-id="4d9a7-403">Test rzeczywisty (stan) w nawiasach i zwraca wartość PRAWDA lub FAŁSZ.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-403">The actual test (condition) is in parentheses and returns true or false.</span></span> <span data-ttu-id="4d9a7-404">Instrukcje, które uruchomienia, jeśli test jest wartość true, są ujęte w nawiasy klamrowe.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-404">The statements that run if the test is true are enclosed in braces.</span></span> <span data-ttu-id="4d9a7-405">`if` Może zawierać instrukcji `else` blok, który określa instrukcje do uruchomienia, jeśli warunek nie jest spełniony:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-405">An `if` statement can include an `else` block that specifies statements to run if the condition is false:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample50.cshtml)]

<span data-ttu-id="4d9a7-406">Można dodać wiele warunków przy użyciu `else if` bloku:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-406">You can add multiple conditions using an `else if` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample51.cshtml)]

<span data-ttu-id="4d9a7-407">W tym przykładzie, jeśli pierwszy warunek w instrukcji if bloku nie ma wartość true, `else if` warunek jest sprawdzany.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-407">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="4d9a7-408">Jeśli ten warunek jest spełniony, instrukcje w `else if` bloku są wykonywane.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-408">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="4d9a7-409">Jeśli żaden z warunków nie jest spełniony, instrukcje w `else` bloku są wykonywane.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-409">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="4d9a7-410">Można dodać dowolną liczbę else if blokuje, a następnie Zamknij z `else` zablokować, jako &quot;cała reszta&quot; warunku.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-410">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="4d9a7-411">Aby przetestować dużą liczbę warunków, należy użyć `switch` bloku:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-411">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="4d9a7-412">Wartość do sprawdzenia jest w nawiasach (w tym przykładzie `weekday` zmiennej).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-412">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="4d9a7-413">Każdy pojedynczy test używa `case` instrukcję, która kończy się wraz z dwukropkiem (:).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-413">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="4d9a7-414">Jeśli wartość `case` instrukcji pasuje do wartości testu, wykonywany jest kod, w tym przypadku bloku.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-414">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="4d9a7-415">Zamknij każdej instrukcji case z `break` instrukcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-415">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="4d9a7-416">(Jeśli zapomnisz o wstawieniu podziału w każdym `case` bloku kodu z następnej `case` instrukcji zostaną również uruchomione.) A `switch` blok często ma `default` instrukcję jako ostatni przypadku &quot;cała reszta&quot; opcja, która jest uruchamiana, gdy żaden z pozostałych przypadkach jest spełniony.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-416">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="4d9a7-417">Wynik ostatnich dwóch bloków warunkowych, wyświetlany w przeglądarce:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-417">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="4d9a7-419">Tworzenie pętli kodu</span><span class="sxs-lookup"><span data-stu-id="4d9a7-419">Looping Code</span></span>

<span data-ttu-id="4d9a7-420">Często muszą uruchomić te same instrukcje wielokrotnie.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-420">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="4d9a7-421">W tym pętli są uwzględnione.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-421">You do this by looping.</span></span> <span data-ttu-id="4d9a7-422">Na przykład można często uruchamiane te same instrukcje dla każdego elementu w kolekcji danych.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-422">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="4d9a7-423">Jeśli wiesz, dokładnie tak jak często chcesz pętli, możesz użyć `for` pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-423">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="4d9a7-424">Ten rodzaj pętli jest szczególnie przydatna podczas liczenia w lub inwentaryzacji:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-424">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="4d9a7-425">Pętla zaczyna się od `for` — słowo kluczowe następują trzy instrukcje w nawiasach, każdy kończyć się średnikiem.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-425">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="4d9a7-426">Wewnątrz nawiasów w pierwszej instrukcji (`var i=10;`) tworzy licznika i inicjuje ją do 10.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-426">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="4d9a7-427">Nie masz nazwy licznika `i` &#8212; można użyć dowolnej zmiennej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-427">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="4d9a7-428">Gdy `for` kiedy pętla zostaje uruchomiona, licznik jest automatycznie zwiększany.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-428">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="4d9a7-429">Druga instrukcja (`i < 21;`) określa warunek, o ile liczba.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-429">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="4d9a7-430">W tym przypadku chcesz przejść do maksymalnie 20 (czyli kontynuować pracę podczas gdy ten licznik jest mniej niż 21).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-430">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="4d9a7-431">Trzecia instrukcja (`i++` ) używa operator inkrementacji, który po prostu Określa, że licznik powinny mieć 1 dodawanych do niego przy każdym uruchomieniu pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-431">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="4d9a7-432">Wewnątrz nawiasów klamrowych jest kod, który będzie wykonywany dla każdej iteracji pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-432">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="4d9a7-433">Znaczniki, tworzy nowy akapit (`<p>` elementu) każdego czas i dodaje wiersz danych wyjściowych, wyświetlanie wartości `i` (licznik).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-433">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="4d9a7-434">Po uruchomieniu tej strony, ten przykład tworzy 11 wiersza, wyświetlanie danych wyjściowych z tekstem w każdym wierszu, wskazującą liczbę elementów.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-434">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="4d9a7-436">Jeśli pracujesz z kolekcji lub tablicy, są często używane `foreach` pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-436">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="4d9a7-437">Kolekcja to grupa podobnych obiektów i `foreach` pętli pozwala przeprowadzić zadanie dla każdego elementu w kolekcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-437">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="4d9a7-438">Ten typ pętli jest wygodne w przypadku kolekcji, ponieważ w odróżnieniu od `for` pętli, nie trzeba ten licznik lub Ustaw limit.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-438">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="4d9a7-439">Zamiast tego `foreach` kodu pętli po prostu obejmującego kolekcji dopiero po jej zakończeniu.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-439">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="4d9a7-440">Na przykład, poniższy kod zwraca elementy w `Request.ServerVariables` kolekcji, która jest obiektem, który zawiera informacje o serwerze sieci web.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-440">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="4d9a7-441">Używa ona `foreac` h pętli, aby wyświetlić nazwę każdego elementu przez utworzenie nowego `<li>` element na liście punktowanej we wcześniejszej HTML.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-441">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="4d9a7-442">`foreach` — Słowo kluczowe, następuje nawiasy, o których należy zadeklarować zmienną, która reprezentuje pojedynczy element w kolekcji (w tym przykładzie `var item`), a następnie `in` — słowo kluczowe, następuje kolekcję ma pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-442">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="4d9a7-443">W treści `foreach` pętli, możesz uzyskać dostęp bieżącego elementu przy użyciu zmiennej zadeklarowanej wcześniej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-443">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="4d9a7-445">Aby utworzyć bardziej ogólnego przeznaczenia pętli, użyj `while` instrukcji:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-445">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="4d9a7-446">A `while` pętli zaczyna się od `while` — słowo kluczowe, następuje nawiasów której określić, jak długo będzie nadal pętli (tutaj, aby uzyskać tak długo, jak `countNum` jest mniejsza niż 50), następnie bloku powtórzeń.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-446">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="4d9a7-447">Zazwyczaj Zwiększ pętli (dodać) lub dekrementacji (odjęcia od) zmiennej lub obiektu użytego do zliczania.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-447">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="4d9a7-448">W tym przykładzie `+=` operator dodaje 1- `countNum` przy każdym uruchomieniu pętli.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-448">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="4d9a7-449">(W celu dekrementacja zmiennej w pętlę, który odlicza, należy użyć operatora dekrementacyjnego `-=`).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-449">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="4d9a7-450">Obiekty i kolekcje</span><span class="sxs-lookup"><span data-stu-id="4d9a7-450">Objects and Collections</span></span>

<span data-ttu-id="4d9a7-451">Prawie wszystko w witrynie internetowej platformy ASP.NET jest obiektem, w tym strona sieci web.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-451">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="4d9a7-452">W tej sekcji omówiono niektóre ważne obiekty, które często będziesz pracować z w kodzie.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-452">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="4d9a7-453">Obiekty strony</span><span class="sxs-lookup"><span data-stu-id="4d9a7-453">Page Objects</span></span>

<span data-ttu-id="4d9a7-454">Najbardziej podstawowa obiektu w programie ASP.NET: jest to strona.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-454">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="4d9a7-455">Możesz uzyskać dostęp właściwości obiektu strony bezpośrednio, bez dowolnych obiektów kwalifikujących.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-455">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="4d9a7-456">Poniższy kod pobiera ścieżkę do pliku, przy użyciu `Request` obiekt strony:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-456">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="4d9a7-457">Aby stał się jasne, możesz teraz odwołuje się do właściwości i metody dla bieżącego obiektu strony możesz opcjonalnie użyć słowa kluczowego `this` obiekt strony w kodzie.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-457">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="4d9a7-458">Poniżej przedstawiono w poprzednim przykładzie kodu za pomocą `this` dodane do reprezentowania strony:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-458">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="4d9a7-459">Można użyć właściwości `Page` można uzyskać mnóstwo informacji, takich jak:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-459">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="4d9a7-460">`Request`.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-460">`Request`.</span></span> <span data-ttu-id="4d9a7-461">Jak już przeczytane, to zbiór informacji o bieżącym żądaniu, w tym, jakiego rodzaju przeglądarki wysłał żądanie, adres URL strony, tożsamość użytkownika, itp.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-461">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="4d9a7-462">`Response`.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-462">`Response`.</span></span> <span data-ttu-id="4d9a7-463">Jest to zbiór informacji na temat odpowiedzi (strona), który będzie wysyłany do przeglądarki, gdy kod serwera zakończył działanie.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-463">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="4d9a7-464">Na przykład można używasz tej właściwości można zapisać informacji do odpowiedzi.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-464">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="4d9a7-465">Obiekty kolekcji (tablic i słowników)</span><span class="sxs-lookup"><span data-stu-id="4d9a7-465">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="4d9a7-466">A *kolekcji* jest grupą obiektów tego samego typu, na przykład zbiór `Customer` obiektów z bazy danych.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-466">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="4d9a7-467">Program ASP.NET zawiera wiele kolekcji wbudowanych, takie jak `Request.Files` kolekcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-467">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="4d9a7-468">Często będziesz pracować z danymi w kolekcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-468">You'll often work with data in collections.</span></span> <span data-ttu-id="4d9a7-469">Istnieją dwa typy kolekcji typowe *tablicy* i *słownika*.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-469">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="4d9a7-470">Tablica jest przydatne, gdy ma być przechowywany w kolekcji podobnych elementów, ale nie chcesz utworzyć osobne zmienną do przechowywania każdego elementu:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-470">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="4d9a7-471">W przypadku tablic, możesz zadeklarować na określony typ danych, takich jak `string`, `int`, lub `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-471">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="4d9a7-472">Aby wskazać, że zmienna może zawierać tablicę, Dodaj nawiasów do deklaracji (takie jak `string[]` lub `int[]`).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-472">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="4d9a7-473">Możesz uzyskać dostęp elementów w tablicy przy użyciu ich pozycji (indeks) lub za pomocą `foreach` instrukcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-473">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="4d9a7-474">Indeksy tablicy są oparte na zerze &#8212; oznacza to, pierwszy element jest na pozycji 0, drugi element znajduje się na pozycji 1 i tak dalej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-474">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="4d9a7-475">Można określić liczbę elementów w tablicy, uzyskując jego `Length` właściwości.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-475">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="4d9a7-476">Aby uzyskać położenie konkretny element w tablicy (na przykład aby wyszukać), użyj `Array.IndexOf` metody.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-476">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="4d9a7-477">Można również wykonać elementów, takich jak wstecznego zawartość tablicy ( `Array.Reverse` metoda) i sortować zawartość ( `Array.Sort` metody).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-477">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="4d9a7-478">Dane wyjściowe kodu tablicy ciąg wyświetlany w przeglądarce:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-478">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="4d9a7-480">Słownik to kolekcja par klucz wartość, określane klucz (lub nazwę) można ustawić lub pobrać odpowiednią wartość:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-480">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="4d9a7-481">Aby utworzyć słownik, należy użyć `new` — słowo kluczowe, aby wskazać, czy tworzysz nowy obiekt słownika.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-481">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="4d9a7-482">Słownik można przypisać do zmiennej za pomocą `var` — słowo kluczowe.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-482">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="4d9a7-483">Możesz określać typy danych elementów w słowniku przy użyciu nawiasów ( `< >` ).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-483">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="4d9a7-484">Na końcu deklaracji należy dodać parę nawiasów, ponieważ jest to rzeczywiście metodę, która tworzy nowy słownik.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-484">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="4d9a7-485">Aby dodać elementy do słownika, możesz wywołać `Add` metody na zmienną słownika (`myScores` w tym przypadku), a następnie określ klucz i wartość.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-485">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="4d9a7-486">Alternatywnie można nawiasami kwadratowymi klucza, a następnie wykonaj przypisanie proste, jak w poniższym przykładzie:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-486">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="4d9a7-487">Aby uzyskać wartość ze słownika, należy określić klucz w nawiasach:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-487">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="4d9a7-488">Wywołanie metody z parametrami</span><span class="sxs-lookup"><span data-stu-id="4d9a7-488">Calling Methods with Parameters</span></span>

<span data-ttu-id="4d9a7-489">Podczas czytania we wcześniejszej części tego artykułu, obiekty, które program za pomocą może mieć metody.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-489">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="4d9a7-490">Na przykład `Database` obiekt może mieć `Database.Connect` metody.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-490">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="4d9a7-491">Wiele metod również mieć co najmniej jeden parametr.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-491">Many methods also have one or more parameters.</span></span> <span data-ttu-id="4d9a7-492">A *parametru* jest wartością, który jest przekazywany do metody, aby włączyć metodę w celu zakończenia zadania.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-492">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="4d9a7-493">Na przykład Przyjrzyj się deklarację dla `Request.MapPath` metody, która przyjmuje trzy parametry:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-493">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="4d9a7-494">(Wiersz został opakowany w celu poprawienia czytelności.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-494">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="4d9a7-495">Należy pamiętać, umieścić podziały wierszy niemal dowolnym miejscu, z wyjątkiem wewnątrz ciągów, które są ujęte w znaki cudzysłowu.)</span><span class="sxs-lookup"><span data-stu-id="4d9a7-495">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="4d9a7-496">Ta metoda zwraca ścieżkę fizyczną na serwerze, który odnosi się do określonej ścieżki wirtualnej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-496">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="4d9a7-497">Są trzy parametry dla metody `virtualPath`, `baseVirtualDir`, i `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-497">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="4d9a7-498">(Zwróć uwagę, że w deklaracji, parametry są wyświetlane z typami danych, który będzie mógł zaakceptować). Po wywołaniu tej metody należy podać wartości dla wszystkich trzech parametrów.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-498">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="4d9a7-499">Składnia Razor daje dwie opcje do przekazywania parametrów do metody: *parametry pozycyjne* i *nazwanych parametrów*.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-499">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="4d9a7-500">Aby wywołać metodę, przy użyciu parametry pozycyjne, parametry są przekazywane w kolejności strict, który jest określony w deklaracji metody.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-500">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="4d9a7-501">(Będzie zazwyczaj wiesz to zamówienie, przeczytaj dokumentację dla metody.) Należy wykonać, kolejność i nie można pominąć dowolny z parametrów &#8212; Jeśli to konieczne, możesz przekazać pusty ciąg (`""`) lub `null` parametru pozycyjnych, które nie mają wartości.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-501">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="4d9a7-502">W poniższym przykładzie założono, masz folder o nazwie *skrypty* w witrynie sieci Web.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-502">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="4d9a7-503">Kod wywołuje `Request.MapPath` metody i przekazuje wartości dla trzech parametrów w odpowiedniej kolejności.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-503">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="4d9a7-504">Następnie wyświetla ścieżki wynikowej zamapowany.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-504">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="4d9a7-505">Gdy metoda ma wiele parametrów, możesz przechowywać swój kod bardziej czytelny przy użyciu nazwanych parametrów.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-505">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="4d9a7-506">Aby wywołać metodę, przy użyciu nazwanych parametrów, należy określić nazwę parametru, po której następuje dwukropek (:), a następnie wartość.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-506">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="4d9a7-507">Zaletą nazwanych parametrów jest to, że można przekazać je w dowolnej kolejności, które mają.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-507">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="4d9a7-508">(Wadą jest wywołanie metody nie jest najmniejszy).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-508">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="4d9a7-509">Poniższy przykład wywołuje tę samą metodę jak powyżej, ale korzysta z nazwanych parametrów, aby podać wartości:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-509">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="4d9a7-510">Jak widać, parametry są przekazywane w innej kolejności.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-510">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="4d9a7-511">Jednak po uruchomieniu poprzedniego przykładu i w tym przykładzie zostanie zwrócona taką samą wartość.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-511">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="4d9a7-512">Obsługa błędów</span><span class="sxs-lookup"><span data-stu-id="4d9a7-512">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="4d9a7-513">Instrukcje try-Catch</span><span class="sxs-lookup"><span data-stu-id="4d9a7-513">Try-Catch Statements</span></span>

<span data-ttu-id="4d9a7-514">Często masz instrukcji w kodzie, który może zakończyć się niepowodzeniem ze względów poza Twoją kontrolą.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-514">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="4d9a7-515">Na przykład:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-515">For example:</span></span>

- <span data-ttu-id="4d9a7-516">Jeśli kod próbuje utworzyć lub uzyskać dostęp do pliku, mogą wystąpić wszelkiego rodzaju błędów.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-516">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="4d9a7-517">Żądany plik nie istnieje, może być zablokowana, kod może nie mieć uprawnień i tak dalej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-517">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="4d9a7-518">Podobnie jeśli kod próbuje zaktualizować rekordy w bazie danych, może to być problemy z uprawnieniami, mogą być opuszczane połączenia z bazą danych, dane w celu zaoszczędzenia może być nieprawidłowy i tak dalej.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-518">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="4d9a7-519">W sposób programowania, są nazywane tych sytuacji *wyjątki*.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-519">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="4d9a7-520">Jeśli Twój kod napotka wyjątek, generuje (zgłasza) komunikat o błędzie przykład w najlepszym irytujących dla użytkowników:</span><span class="sxs-lookup"><span data-stu-id="4d9a7-520">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="4d9a7-522">W sytuacjach, w którym kod może wystąpić wyjątki i w celu uniknięcia komunikaty o błędach tego typu można użyć `try/catch` instrukcji.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-522">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="4d9a7-523">W `try` instrukcji, uruchom kod, który jest sprawdzany.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-523">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="4d9a7-524">W co najmniej jednej `catch` instrukcji, można wyszukać konkretnego błędów (określonych typów wyjątków), które mogły wystąpić.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-524">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="4d9a7-525">Może zawierać tyle `catch` instrukcje, jak należy sprawdzić występowanie błędów, które są przewidywania.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-525">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="4d9a7-526">Firma Microsoft zaleca, unikaj używania `Response.Redirect` method in Class metoda `try/catch` instrukcji, ponieważ może to spowodować wyjątek na stronie.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-526">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="4d9a7-527">Poniższy przykład pokazuje stronę, która tworzy plik tekstowy na pierwsze żądanie, a następnie wyświetla przycisk, który umożliwia użytkownikowi, otwórz plik.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-527">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="4d9a7-528">W przykładzie użyto celowo Zła nazwa pliku, tak aby spowoduje wyjątek.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-528">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="4d9a7-529">Kod zawiera `catch` instrukcji dla dwóch możliwych wyjątków: `FileNotFoundException`, który występuje, jeśli nazwa pliku jest nieprawidłowa, i `DirectoryNotFoundException`, który występuje, gdy program ASP.NET nawet nie można odnaleźć folderu.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-529">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="4d9a7-530">(Aby zobaczyć, jak działa, gdy wszystko będzie działać prawidłowo można komentarz do instrukcji w tym przykładzie).</span><span class="sxs-lookup"><span data-stu-id="4d9a7-530">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="4d9a7-531">Jeśli Twój kod nie obsługuje wyjątek, widział takich jak poprzedniej zrzut ekranu przedstawiający stronę błędu.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-531">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="4d9a7-532">Jednak `try/catch` sekcji uniemożliwia użytkownikowi widzisz błędy tych typów.</span><span class="sxs-lookup"><span data-stu-id="4d9a7-532">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="4d9a7-533">Dodatkowe zasoby</span><span class="sxs-lookup"><span data-stu-id="4d9a7-533">Additional Resources</span></span>

<span data-ttu-id="4d9a7-534">**Programowanie za pomocą języka Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="4d9a7-534">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="4d9a7-535">Dodatek: Język Visual Basic i składnia</span><span class="sxs-lookup"><span data-stu-id="4d9a7-535">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="4d9a7-536">**Dokumentacja referencyjna**</span><span class="sxs-lookup"><span data-stu-id="4d9a7-536">**Reference Documentation**</span></span>

[<span data-ttu-id="4d9a7-537">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="4d9a7-537">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="4d9a7-538">W języku C#</span><span class="sxs-lookup"><span data-stu-id="4d9a7-538">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
