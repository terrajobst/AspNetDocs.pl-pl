...
Aby aparat programu ASP.NET do obsługi żądania dla tej strony, strony, fragment kodu ( `WebPage.aspx.cs` pliku) najpierw muszą być skompilowane. Ta kompilacja może się zdarzyć, jawnie lub automatycznie.
...
Jakie podejście kompilacji zależy od, w całości, jak aplikacja ASP.NET jest zarządzany w programie Visual Studio. Od czasu. Incepcja NET w roku 2000 były cztery różne wersje programu Visual Studio — programu Visual Studio .NET 2002, Visual Studio .NET 2003, Visual Studio 2005 i Visual Studio 2008. Visual Studio .NET 2002 i 2003 zarządzane aplikacje ASP.NET z wykorzystaniem *modelu projektu aplikacji sieci Web*. Dostępne są następujące kluczowe funkcje modelu projektu aplikacji sieci Web:
...
Firma Microsoft opublikowała program Visual Studio 2005 porzucony obsługę modelu projektu aplikacji sieci Web i zastąpiono ją przy użyciu modelu projektu witryny sieci Web. Model projektu witryny sieci Web ze zróżnicowanych sam z modelu projektu aplikacji sieci Web w następujący sposób:
...
Nie trzeba kopiować plików kodu źródłowego stron ASP.NET do środowiska produkcyjnego, ani nie trzeba kopiować `BasePage.cs` pliku klasy.
