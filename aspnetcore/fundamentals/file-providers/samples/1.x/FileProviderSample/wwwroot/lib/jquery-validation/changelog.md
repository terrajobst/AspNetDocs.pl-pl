...
## <a name="additional"></a>Dodatkowe
  * Dostosuj cifES i nieES algorytmów. Zamyka #1826

## <a name="build"></a>Kompilacja
  * Dołączyć zminimalizowany wersji pakietu NPM
  * Zwiększyć dev zależności do najnowszej wersji

## <a name="core"></a>Core
  * Dodaj powiązanie dla danych wejściowych za pomocą typu przycisku. Zamyka #1891
  * Obsługuje jquery3. Zamyka #1866
  * Zmiana jQuery aliasu "wyrażenie [": "]" do "expr.pseudos"

## <a name="localisation"></a>Umiejscowienie
  * Dodaj tłumaczenie Urdu. Zamyka #1873.

## <a name="localization"></a>Lokalizacja
  * Naprawiono problem — rozszerzenie pliku dla az tłumaczenia. Zamyka #1890.
  * Dodano brakujące tłumaczenia w pt-BR (zamyka 1897 #)
  * Naprawiono błąd pisowni w pliku języka arabien.

## <a name="tests"></a>Testy
  * Uaktualnij QUnit 2.0.

## <a name="umd"></a>UMD
  * Lepsza obsługa CommonJS.

<a name="1151--2016-07-22"></a>1.15.1 / 2016-07-22
==================

## <a name="additional"></a>Dodatkowe
  * Rozwiązywanie wielu Sprawdzanie typu mime
  * Numer IBAN wymagają co najmniej 5 znaków (zamyka 1797 #, rozwiązuje 1674 #)
  * Poprawne notEqualTo jQuery odwołania

## <a name="core"></a>Core
  * Dodano niepowodzenie testu dla #1805
  * Napraw sprawdzanie poprawności grupy z 3 i więcej pól
  * Napraw regresji wprowadzone w #1644 i 1657 # (zamyka #1800)
  * Weryfikacja kroku aktualizacji poprawnie obsłużyć zmiennoprzecinkowy punktów
  * Usuń błąd podczas wywoływania $.fn.rules() w elemencie — do formularza
  * Wywołaj `errorPlacement` w zakresie modułu sprawdzania poprawności
  * Rozwiązano problem z elementami contenteditable w formularzach, gdzie zdarzeń dla pojedynczego walidacji danych wejściowych spowodowałoby wyjątków

## <a name="demo"></a>Pokaz
  * Dodawanie łączy do ładowania początkowego i interfejsu użytkownika semantycznej pokazów index.html
  * Użyj `.on()` zamiast `.validateDelegate()`

## <a name="localization"></a>Lokalizacja
  * Dodano język Azerbejdżański

## <a name="tests"></a>Testy
  * Dodano regresji testów jednostkowych dla 1760 # żądania Ściągnięcia

<a name="1150--2016-02-24"></a>1.15.0 / 2016-02-24
==================

## <a name="all"></a>Wszystkie
  * Problemy dotyczące stylu stałych kodu

## <a name="core"></a>Core
  * `resetForm` należy również usunąć `valid` klasy z elementów.
  * Pole unhighlighting, jeśli już wyróżniona, gdy przy użyciu zdalnego reguły.
  * Powiąż `blur` zdarzenia tylko raz w `equalTo` reguły
  * Naprawiono błąd podczas wywoływania .rules() na pusty jquery ustawień.
  * Napraw Obsługa komunikatów o błędach z grupami danych wejściowych.
  * Napraw TypeError w `showLabel` przy użyciu `groups` ustawienia
  * Dodawanie sposobem przekazywania Nazwa metody do zdalnego
  * Sprawdzanie poprawności nie powiedzie się wyzwolić podczas następnego pola są już wypełnione (poprawki 1508 #)
  * Wymagana reguła mają wyższy priorytet nad cyfr & liczby reguł
  * Błąd ukryte ale klasy błędów wejścia nie usunięty
  * Komunikaty o błędach niewłaściwego korzysta z zdalnej weryfikacji
  * Pole stałej wyróżnianie za pomocą zdalnej weryfikacji.
  * Naprawiono `:filled` selektora dla wielu wybranych elementów.
  * Dodano doc odwołanie do jQuery.validator.methods
  * Przenieś wiadomość, przetwarzając je z `formatAndAdd` do `defaultMessage`
  * ErrorList może zawierać tylko błędy, które powinien
  * Wyodrębnij nazwę pliku bez uwzględniania "C:\fakepath\"
  * Obsługa atrybutu kroku HTML5. Poprawki #1295
  * Dodano obsługę "oczekujące" klasy na oczekujących żądań
  * Dodano normalizer (#1602)
  * Podziel się `creditcard` — metoda
  * Znak ucieczki Identyfikator błędu do użycia w wyrażeń regularnych, aby nie tworzyć aria describedby
  * Znak ucieczki apostrofy w nazwach unikanie jest zgłoszony błąd pójdzie jak z płatka
  * Zamiast używania wartości sprawdzania poprawności pola, aby pominąć wywołania interfejsu api, użyj obiektu serializowane dane żądania
  * Dodano obsługę contentEditable tagów

## <a name="additional"></a>Dodatkowe
  * Kod identyfikacyjny: Zezwalaj na cyfry 1-9, w drugim miejscu lokalizacji
  * Zaakceptuj, że metoda wyrażenia regularnego powinno być poprzedzone znakiem zmiany znaczenia prawidłowo.
  * Bez uwzględniania wielkości liter, Wyszukaj kod identyfikacyjny
  * Poprawne postalCodeCA, które mają zostać wykluczone nieprawidłowe kombinacje
  * Wprowadź więcej łagodne postalCodeCA — metoda

## <a name="localization"></a>Lokalizacja
  * Dodano Macedoński lokalizacji.
  * Dodano Brak komunikatu wzorca w Polski (adamwojtkiewicz)
  * Stały perski tłumaczenie wiadomości minimalnej/maksymalnej.
  * Zaktualizowano messages_sk.js
  * Aktualizuj Malay tłumaczenia
  * Uwzględnione wiadomości z dodatkowych metod
  * Poprawa pt_BR tłumaczenia i poprawienie pisowni w kluczu "cifES".
...
==================
  ...
  * Należy valid() zwracać wartość logiczną, a nie 0 lub 1. Poprawki #109 - valid() nie może zwracać wartość logiczną
...
* Nie używaj form.elements, ponieważ już, która nie jest obsługiwana w interfejs jQuery 1.6. Jego buggy jako hell mimo to (IE6-8: form.elements === formularza).
...
* Naprawiono opcjonalne — metody nie Wyróżnij nic w ogóle, jeśli to pole jest puste, oznacza to, nie wyzwolenia sukces
...
* Całkowicie usunięte zależność od identyfikatory elementów, jednak są nadal używane do połączyć błąd etykiet danych wejściowych (jeśli istnieją). Osiągnąć przy użyciu tablicy {nazwa, wiadomości, element} zamiast obiekt z parami id: komunikat dla wewnętrznego errorList.
